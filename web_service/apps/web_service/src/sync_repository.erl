...
add( Entity ) ->
    F = fun() -> 
        FrontCars = db_manager:select( sync_entity, #sync_entity{ 
                                                                    side = Entity#sync_entity.side, 
                                                                    rear_car = undefined, 
                                                                    _ = '_'
                                                                }, [ ], ['$_'] ),
        Return = if length( FrontCars ) == 1 ->
            ...
            FrontCar;
        true ->
            OppositeCars = db_manager:select( sync_entity, #sync_entity{ 
                                                                            side = ( Entity#sync_entity.side * -1 ), 
                                                                            _ = '_' 
                                                                        }, [ ], ['$_'] ),
        ...
        if Return =/= [ ] ->
            db_manager:add( Entity#sync_entity{ 
                                                time_stamp = Index * Entity#sync_entity.side, 
                                                rear_car = undefined, 
                                                front_car = Return#sync_entity.name 
                                              } );
        true ->
            db_manager:add(Entity#sync_entity{ 
                                                time_stamp = Index * Entity#sync_entity.side, 
...
delete( Entity ) ->
    F = fun() -> 
        Result = repository_helper:select( sync_entity, #sync_entity{ 
                                                                        name = Entity#sync_entity.name, 
                                                                        _ = '_' 
                                                                    }, [ ], ['$_'] ),
        if length(Result) == 1 ->
            [ Item ] = Result,
            ...
            RearCar = if Item#sync_entity.rear_car =/= undefined ->
                ...
                RearCars = repository_helper:select( sync_entity, #sync_entity{ 
                                                                                name = Item#sync_entity.rear_car, 
                                                                                _='_' 
                                                                               }, [ ], ['$_'] ),
            ...
            FrontCar = if Item#sync_entity.front_car =/= undefined ->
                ...
                FrontCars = repository_helper:select( sync_entity, #sync_entity{ 
                                                                                    name = Item#sync_entity.front_car, 
                                                                                    _='_'
                                                                                }, [ ], ['$_'] ),
