...
adj( Entity ) ->
    ...
    if Entity#adj_entity.state == stop ->
        ...
        sync_repository:delete( #sync_entity{ 
                                                name = Entity#adj_entity.name, 
                                                side = Entity#adj_entity.side, 
                                                power = Entity#adj_entity.power
                                            } );
    true ->
        ok 
    end,
...
kill( Name, Target ) ->
    ...
    if length( SelectedCars ) == 1 -> 
        ...
        sync_repository:delete( #sync_entity{ 
                                                name = SelectedCar#adj_entity.name, 
                                                side = SelectedCar#adj_entity.side, 
                                                power = SelectedCar#adj_entity.power
                                            } ),
