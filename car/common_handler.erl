...
compute_position( Data, Front ) ->
  % compute car position
  TravelTime = ( utils:get_timestamp() - current_time(Data) ) / 1000,
  Position = position(Data) + ( speed(Data) * TravelTime * ( -1 * side(Data) ) ),
  utils:log( "Compute position: Travel Time ( s ): ~p, Speed: ~p, OldPosition: ~p, CurrentPosition: ~p, Crossing ~p", [ TravelTime, speed(Data), position(Data), Position, crossing(Data) ] ),
  Position.
...
propagate_crossing_wrapper( Data, RearCars, Body ) -> 
  if length( RearCars ) > 0, Body#car_state.bridge_capacity > 0 ->
    ...
    car_call_supervisor_api:car_call( { crossing, name(Data), name(Pivot), max_RTT(Data), NewBody } ),
