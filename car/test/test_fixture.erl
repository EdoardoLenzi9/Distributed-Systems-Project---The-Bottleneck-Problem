...
default_state2() ->
    ( default_state() )#car_state{
                                    adj = #adj{ 
                                                front_cars = [ #car_state{ 
                                                                            name = node(), 
                                                                            side = -1, 
                                                                            position = -1, 
                                                                            size = 1 
...
listen( Label, Handler ) ->
    receive
        ...
        { car_call, Req } ->
            ...
            case ReqLabel of 
                Label ->
                    utils:log("Test: result ~p ", [ Handler( 
                                                             ReqLabel, 
                                                             ReqSender, 
                                                             ReqTarget, 
                                                             ReqRTT, 
...
skip_sync3( State ) ->
    ...
    listen( check, fun( _, _, _, _, _ ) -> 
        car:check_reply( { 
                            ...
                            utils:get_timestamp(), 
                            0, 
                            State#car_state{ current_time = utils:get_timestamp() } 
                        } )
    end ),
    listen( adj, fun( _, Sender, _, _, _ ) -> 
        Adj = #adj{ 
...
skip_leader3( _State ) ->   
    ...
    listen( check_reply, fun( _ReplyLabel, ReplySender, ReplyTarget, ReplySendingTime, ReplyBody ) -> 
        ...
        { _Result, _Data } = car:check_reply( { 
                                                ReplySender, 
                                                ReplyTarget, 
                                                ReplySendingTime, 
                                                RTT, 
                                                ReplyBody#car_state{ 
                                                                     arrival_time = ReplySendingTime 
                                                                    } 
                                            } )
    end ).
