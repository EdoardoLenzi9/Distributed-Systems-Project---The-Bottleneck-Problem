...
In order to detacht any type of failure we defined different $scenerys$: these are json files which instantiate
...
\begin{itemize}
    ...
    \end{center}
    \item[scenery \textbf{8}]: \\ there are five cars on the same side but only one (with the longer delay) is sane. \\
    ...
    \end{center} 
    \item[scenery \textbf{9}]: \\ there are three cars, two from the same side. One of them has a system crash 
